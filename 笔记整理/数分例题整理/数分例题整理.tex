...
\newcommand{\nti}{\lim_{n \to \infty}}
\newcommand{\infnti}{\varliminf_{n \to \infty}}
\newcommand{\supnti}{\varlimsup_{n \to \infty}}
% ————————————————————————————————————自定义符号————————————————————————————————————
...
\begin{dl}[积分第二中值定理]
    $f(x)$ 在 $[a, b]$ 可积， $g(x)$ 在 $[a, b]$ 上单调有界. 则存在 $\xi \in [a, b]$，使得 
    \[
...
\begin{dy}[函数的 $p$-范数]
    对 $[a, b]$ 上可积的函数 $f(x)$, 定义 $f(x)$ 的 $p$-范数：
    \[
        \left\lVert f(x)\right\rVert_p = \left(\int_{a}^{b}\left\lvert f(x)\right\rvert ^p \dx \right)^\frac{1}{p}
    \]
\end{dy}

\begin{dl}[$\text{H}\ddot{\text{o}}\text{lder}$ 不等式]
    $f(x), g(x)$ 在 $[a, b]$ 上可积，且有 $p, q > 1$ 且 $\frac{1}{p} + \frac{1}{q} = 1$, 则有
    \[
        \int_{a}^{b}f(x)g(x)\dx \leqslant \left(\int_{a}^{b}\left\lvert f(x)\right\rvert^p \dx \right)^\frac{1}{p} \cdot \left(\int_{a}^{b}\left\lvert g(x)\right\rvert^q\dx \right)^\frac{1}{q}
    \]
    即 
    \[
        \int_{a}^{b}f(x)g(x)\dx \leqslant \left\lVert f(x)\right\rVert_p \cdot \left\lVert g(x)\right\rVert_q
    \]
    当 $p = q = 2$ 时，即 Cauchy-Schwarz 不等式：
    \[
        \left(\int_{a}^{b}f(x)g(x)\dx\right)^2 \leqslant \int_{a}^{b}\left\lvert f(x)\right\rvert^2 \dx \cdot \int_{a}^{b}\left\lvert g(x)\right\rvert^2 \dx
    \]
\end{dl}

\begin{zm}
    由 Young 不等式，类似 \ref{离散 holder 证明}离散 $\text{H}\ddot{\text{o}}\text{lder}$ 不等式 即可.
\end{zm}

\begin{lt}
    设函数 $f(x)$ 在区间 $[a, b]$ 上有连续导数，$f(a)=f(b)=0$，且 $\int_{a}^{b} f(x)^2 \dx = 1$. 证明： 
    \[
        \int_{a}^{b} x^2 (f'(x))^2 \dx \geq \frac{1}{4}
    \]
    \begin{zmhj}
        \begin{align*}
            1 &= \int_{a}^{b} f(x)^2 \dx \\
              &= x f(x)^2 \bigg|_{a}^{b} - \int_{a}^{b} x (f(x)^2)' \dx \\
              &= - \int_{a}^{b} 2x f'(x) f(x) \dx \\
              &= 2 \int_{a}^{b} (-x f'(x)) f(x) \dx \\
              &\leq 2 \left( \int_{a}^{b} (-x f'(x))^2 \dx \right)^{\frac{1}{2}} \left( \int_{a}^{b} f(x)^2 \dx \right)^{\frac{1}{2}} \\
              &= 2 \left( \int_{a}^{b} x^2 (f'(x))^2 \dx \right)^{\frac{1}{2}}
        \end{align*}
        \qed
    \end{zmhj}
\end{lt}

\subsubsection{有关可求长曲线}

\begin{dy}[曲线的定义]
    \begin{itemize}
        \item 参数方程：闭区间 $[a, b]$ 到 $\RR^2$ 的连续映射 $\gamma$
        \item 曲线：映射 $\gamma$ 的象空间
    \end{itemize}
\end{dy}

\begin{dy}[可求长曲线]
    若对 $[a, b]$ 的任意划分 $P: a = t_0 < t_1 < \cdots < t_n = b$, 极限 $\lim_{\lambda(P) \to 0}\sum_{i = 1}^{n}\overline{\gamma(t_{i - 1})\gamma(t_i)} $ 存在且其值不依赖划分的选取. \\
    改极限定义为曲线的长度.
\end{dy}

不难发现 
\[ 
    \lim_{\lambda(P) \to 0}\sum_{i = 1}^{n}\overline{\gamma(t_{i - 1})\gamma(t_i)} = \sup_{P}\sum_{i = 1}^{n}\overline{\gamma(t_{i - 1})\gamma(t_i)}
\]

\begin{dy}[光滑曲线]
    对曲线的参数方程 $\gamma(t) = \left(x(t), y(t)\right)$, 若 $x(t), y(t)$ 均有连续导数，且
    \[
...
\begin{dl}
    若曲线的参数方程的各个分量均可求导且导数可积，则曲线可求长，且其长度为
    \[
...
\subsubsection{无穷积分和瑕积分的定义}

(略)

\begin{lt}
    求积分 $I_n = \int_{0}^{+\infty}x^n\zte^{-x}\dx$
    \begin{jiehj}
        \begin{align*}
            I_n = -x^n\zte^{-x}\bigg|_0^{+\infty} + n\int_{0}^{+\infty}x^{n - 1}\zte^{-x}\dx = nI_{n - 1}
        \end{align*}
        又
        \begin{align*}
            I_0 = \int_{0}^{+\infty}\zte^{-x}\dx = 1
        \end{align*}
        因此 $I_n = n!$
    \end{jiehj}
\end{lt}

我们令 $\Gamma (x) = \int_{0}^{+\infty}t^x\zte^{-t}\dt$

\begin{lt}
    求积分 $I = \int_{0}^{\pi/2}\ln \sin x \dx$.
    \begin{jiehj}
        \begin{align*}
            I &\xlongequal{x = 2t} 2\int_{0}^{\pi/4}\ln 2 \sin t \cos t \dt \\
            &= \frac{\pi}{2}\ln 2 + 2\left(\int_{0}^{\pi/4}\ln \sin x \dx + \int_{0}^{\pi/4}\ln \cos x \dx\right) \\
            &= \frac{\pi}{2}\ln 2 + 2 \int_{0}^{\pi/2}\ln \sin x 
            dx = \frac{\pi}{2}\ln 2 + 2I
        \end{align*}
        因此 $I = -\frac{\pi}{2}\ln 2$
    \end{jiehj}
\end{lt}
...
\begin{lt}
    求积分 $I = \int_{0}^{+\infty}\frac{1}{1 + x^2}\frac{1}{1 + x^\alpha} \dx$.
    \begin{jiehj}
        \begin{align*}
            I &= \int_{0}^{1}\frac{1}{1 + x^\alpha} \dx + \int_{0}^{1}\frac{1}{1 + \frac{1}{t^2}}\frac{1}{1 + \frac{1}{t^\alpha}}\dd \left(\frac{1}{t}\right) \\
            &= \int_{0}^{1}\frac{1}{1 + x^\alpha} \dx + \int_{0}^{1}\frac{t^\alpha \dt}{\left(1 + t^2\right)\left(1 + t^\alpha\right)}\\
            &= \int_{0}^{1}\frac{\dx}{1 + x^2} = \frac{\pi}{4}
        \end{align*}
    \end{jiehj}
\end{lt}

\subsubsection{广义积分敛散性的判别法}

\begin{dl}[比较判别法]
    $f(x), \varphi(x) \geqslant 0$ 在任意有限区间上 Riemann 可积，且
    \[
        \lim_{x \to +\infty}\frac{\varphi(x)}{f(x)} = k \geqslant 0 \text{ 或 } +\infty
    \]
    则
    \begin{itemize}
        \item $k \in [0, +\infty)$, 则 $\int_{a}^{+\infty}\varphi(x)\dx$ 收敛 $\Rightarrow$ $\int_{a}^{+\infty}f(x)\dx$ 收敛.
        \item $k \in (0, +\infty]$, 则 $\int_{a}^{+\infty}f(x)\dx$ 发散 $\Rightarrow$ $\int_{a}^{+\infty}\varphi(x)\dx$ 发散.
    \end{itemize}
    常用 $\varphi(x) = \frac{1}{x^p}$, 是为 Cauchy 判别法.
\end{dl}

\begin{dl}[Abel-Dirichlet 判别法]
    $f(x), \varphi(x) \geqslant 0$ 在任意有限区间上 Riemann 可积，一下条件成立之一时， $\int_{a}^{+\infty}f(x)g(x)\dx$ 收敛
    \begin{itemize}
        \item (Abel) $\int_{a}^{+\infty}f(x)\dx$ 收敛， $g(x)$ 单调有界
        \item (Dirichlet) $F(x) = \int_{a}^{x}f(t)\dt$ 有界， $g(x)$ 单调且收敛到 $0$
    \end{itemize}
\end{dl}
...
\begin{zm}
    ...
    \]
    上述两个条件依次对应 $\int_{A'}^{A''}f(x)\dx$ 小 和 $g(A)$ 小，再利用 Cauchy 收敛原理即可.
\end{zm}
...
\begin{dy}
    $S_n = \sum_{i = 1}^{n}a_i$ 为级数 $\sif a_n$ 的部分和序列.
\end{dy}
...
\begin{dl}
    若级数 $\sif a_n$ 收敛，则在其求和表达式中任意添加括号所得级数任然收敛.
\end{dl}
...
\subsubsection{上下极限的定义与性质}

\begin{dy}
    设 $\{x_n\}$ 是一有界个数列，$\xi \in \mathbb{R}$。如果存在 $\{x_n\}$ 的一个子列 $\{x_{n_k}\}$ 满足
    $$
        \lim_{k \to \infty} x_{n_k} = \xi,
    $$ 
    则称 $\xi$ 是 $\{x_n\}$ 的一个极限点。将 $\{x_n\}$ 一切极限点所组成的集合记作 $E$. $\sup E$ 称为 $\{x_n\}$ 的上极限，记作 $\varlimsup_{n \to \infty} x_n$。$\inf E$ 称为 $\{x_n\}$ 的下极限，记作 $\varliminf_{n \to \infty} x_n$.
\end{dy}

\begin{dl}
    设 $x_n$ 有界，则
    \begin{itemize}
        \item  存在子列 $\{x_{n_i}\}_{i=1}^{\infty}$，使得 $\lim_{i \to \infty} x_{n_i} = \supnti x_n$;
        \item  存在子列 $\{x_{n_k}\}_{k=1}^{\infty}$，使得 $\lim_{k \to \infty} x_{n_k} = \infnti x_n$.
    \end{itemize}
\end{dl}
...
\begin{dl}
    设 $x_n$ 有界，则
    \begin{itemize}
        \item $H = \supnti x_n$ 的充要条件是：对任意给定 $\varepsilon > 0$
        \begin{itemize}
            \item 存在 $N \in \mathbb{N}$，$\forall n \geq N: x_n < H + \varepsilon$;
            \item 数列 $\{x_n\}$ 中有无穷多项大于 $H - \varepsilon$.
        \end{itemize}
        \item $h = \infnti x_n$ 的充要条件是：对任意给定 $\varepsilon > 0$
        \begin{itemize}
            \item 存在 $N \in \mathbb{N}$，$\forall n \geq N: x_n > h - \varepsilon$;
            \item 数列 $\{x_n\}$ 中有无穷多项小于 $h + \varepsilon$.
        \end{itemize}
    \end{itemize}
\end{dl}

\begin{dl}
    $\{x_n\}$ 收敛的充要条件是 $\supnti x_n = \infnti x_n$.
\end{dl}

\begin{dl}
    设 $\{x_n\}, \{y_n\}$ 是两个数列，则
    \begin{itemize}
        \item $\supnti (x_n + y_n) \leq \supnti x_n + \supnti y_n$, \quad $\liminf_{n \to \infty} (x_n + y_n) \geq \infnti x_n + \infnti y_n$;
        \item 若 $\lim_{n \to \infty} x_n$ 存在，则
        $$
            \supnti (x_n + y_n) = \lim_{n \to \infty} x_n + \supnti y_n, \quad \infnti (x_n + y_n) = \lim_{n \to \infty} x_n + \infnti y_n.
        $$ 
    \end{itemize}    
\end{dl} 

\begin{dl}
    $\supnti x_k = \lim_{n \to \infty} \sup_{k \geqslant n} x_k, \quad \infnti x_k = \lim_{n \to \infty} \inf_{k \geqslant n} x_k.$
\end{dl}

\subsubsection{正项级数敛散性的判别法}

\begin{dl}
    正项数列 $\sif a_n$ 收敛的充要条件是其部分和序列是有界序列.
\end{dl}

\begin{dl}[比较判别法]
    设  $\sum_{n=1}^{\infty} x_n$, $\sum_{n=1}^{\infty} y_n$  是正项级数，且存在常数  $A > 0$, 使得
    $$
        x_n \leq A y_n, \quad n \in \mathbb{N}.
    $$ 
    \begin{itemize}
        \item 如果  $\sum_{n=1}^{\infty} y_n$  收敛，则  $\sum_{n=1}^{\infty} x_n$  收敛；
        \item 如果  $\sum_{n=1}^{\infty} x_n$  发散，则  $\sum_{n=1}^{\infty} y_n$  发散。
    \end{itemize}
\end{dl}

\begin{dl}[比较判别法的极限形式]
    设  $\sum_{n=1}^{\infty} x_n$ ， $\sum_{n=1}^{\infty} y_n$  是正项级数，且
    $$
        \lim_{n \to \infty} \frac{x_n}{y_n} = l \in [0, +\infty).
    $$ 
    \begin{itemize}
        \item 若  $0 \leq l < +\infty$ ，则当  $\sum_{n=1}^{\infty} y_n$  收敛时， $\sum_{n=1}^{\infty} x_n$  也收敛；
        \item 若  $0 < l \leq +\infty$ ，则当  $\sum_{n=1}^{\infty} y_n$  发散时， $\sum_{n=1}^{\infty} x_n$  也发散。
    \end{itemize}
\end{dl}

\begin{dl}[Cauchy 判别法]
    设  $\sum_{n=1}^{\infty} x_n$  是正项数列，且  $\lim_{n \to \infty} \sqrt[n]{x_n} = r$ ，则：
    \begin{itemize}
        \item 如果  $r < 1$ ，则  $\sum_{n=1}^{\infty} x_n$  收敛；
        \item 如果  $r > 1$ ，则  $\sum_{n=1}^{\infty} x_n$  发散；
        \item 如果  $r = 1$ ，则无法判断。
    \end{itemize}
\end{dl}

\begin{dl}[d'Alembert 判别法]
    设  $\sum_{n=1}^{\infty} x_n$  是正项数列，且  $x_n \neq 0$ ，则：
    \begin{itemize}
        \item 如果  $\supnti \frac{x_{n+1}}{x_n} = \bar{r} < 1$ ，则  $\sum_{n=1}^{\infty} x_n$  收敛；
        \item 如果  $\infnti \frac{x_{n+1}}{x_n} = \underline{r} > 1$ ，则  $\sum_{n=1}^{\infty} x_n$  发散；
        \item 如果  $\bar{r} \geq 1$  或  $\underbar{r} \leq 1$ ，则无法判断。
    \end{itemize}
\end{dl}

\begin{dl}[积分判别法]
    设  $f(x)$  是  $[a, +\infty)$  上的非负函数，且在任意有限区间  $[a, A] \subset [a, +\infty)$  上 Riemann 可积。取一个严格单增的无穷大量  $\{a_n\}$ ：
    $$
        a = a_1 < a_2 < a_3 < \cdots < a_n < \cdots \to +\infty
    $$ 
    令
    $$
        u_n := \int_{a_n}^{a_{n+1}} f(x) \dx
    $$ 
    则无穷积分  $\int_a^{+\infty} f(x) \dx$  与正项级数  $\sum_{n=1}^{\infty} u_n$  同时收敛或同为  $+\infty$ . \\
    特别地，如果  $f(x)$  单减， $a_n = n$ ，则  $\int_a^{+\infty} f(x) \, dx$  与  $\sum_{n=[a] + 1}^{\infty} f(n)$ 同时收敛或发散.
\end{dl}

\begin{lt}
    级数 $\sif \frac{1}{n^p}$ 在 $0 < p \leq 1$ 时发散，在 $p > 1$ 时收敛.
\end{lt}
...
\begin{dy}[条件收敛和绝对收敛]
    设  $\sum_{n=1}^{\infty} a_n$  是任意项级数，如果正项级数  $\sum_{n=1}^{\infty} |a_n|$  收敛，则称级数  $\sum_{n=1}^{\infty} a_n$  绝对收敛。如果  $\sum_{n=1}^{\infty} a_n$  收敛，但  $\sum_{n=1}^{\infty} |a_n|$  发散，则称级数  $\sum_{n=1}^{\infty} a_n$  条件收敛.
\end{dy}

\begin{dl}
    如果级数 $\sif a_n$ 绝对收敛，则其收敛.
\end{dl}

\subsubsection{Leibniz 判别法}

\begin{dy}[Leibniz 级数]
    设 $\{u_n\}_1^\infty$ 是正项数列，则称 $\sif (-1)^{n + 1}u_n$ 为交错级数. \\
    若 $u_n$ 单减且收敛到 $0$, 则称 $\sif (-1)^{n + 1}$ 为 Leibniz 级数.
\end{dy}
...
\subsubsection{Abel 判别法与 Dirichlet 判别法}
...
\subsubsection{度量空间、内积、范数}

\begin{dy}[度量空间]
    设 $X$ 是一个集合，如果定义在 $X \times X$ 上的映射 $d: X \times X \to \mathbb{R}$ 满足
    \begin{itemize}
        \item 正定性：$d(x, y) \geq 0$，且 $d(x, y) = 0$ 当且仅当 $x = y$；
        \item 对称性：$d(x, y) = d(y, x)$；
        \item 三角不等式：$d(x, z) \leq d(x, y) + d(y, z)$.
    \end{itemize}
    则称 $d$ 为 $X$ 上的一个度量，$(X, d)$ 为一个度量空间.
\end{dy}
...
\subsubsection{聚点、内点、外点和边界点}

\begin{dy}[聚点]
    如果 $\forall \varepsilon > 0$, 有 $A \cap \mathring{B}(x, \varepsilon) \neq \varnothing$, 则称 $x$ 为 $A$ 的一个聚点. \\
    等价地有若存在 $\{x_n\} \in A$ 且 $x_n \neq x$, 成立 $\lim_{n \to \infty}x_n = x$. \\
    所有聚点构成的集合为 $A$ 的导集，记作 $A'$
\end{dy}

\begin{dy}[收敛序列]
    $\{x_n\}_{n = 1}^\infty$ 是度量空间 $X$ 中的一个序列，如果对任意 $\varepsilon > 0$, 存在 $N > 0$, 任意 $n > N$ 有 $d(x_n, x) < \varepsilon$, 则称 $x_n$ 收敛到 $x$.
\end{dy}

\begin{dy}[内点、外点和边界点]
    设 $(X,d)$ 是一个度量空间, $A \subset X$ 是 $X$ 的一个子集，$x \in X$.
    \begin{enumerate}
        \item[(1)] (内点) 如果 $\exists \varepsilon > 0$, 使得 $B(x,\varepsilon) \subset A$，则称 $x$ 为 $A$ 的一个内点. 集合 $A$ 的一切内点所组成的集合称为 $A$ 的内部，记作 $A^\circ$.
        \item[(2)] (外点) 如果 $\exists \varepsilon > 0$, 使得 $B(x,\varepsilon) \cap A = \varnothing$，则称 $x$ 为 $A$ 的一个外点. $A$ 的一切外点组成的集合为 $(A^c)^\circ$.
        \item[(3)] (边界点) 如果 $\forall \varepsilon > 0$, $B(x,\varepsilon) \cap A \neq \varnothing$ 且 $B(x,\varepsilon) \cap A^c \neq \varnothing$，则称 $x \in X$ 为 $A$ 的一个边界点. $A$ 的所有边界点组成的集合记作 $\partial A$.
    \end{enumerate}
    显然，$A$ 的内点集合、外点集合和边界点集合的并集为整个空间 $X$，即
    $$
        A^\circ \cup (A^c)^\circ \cup \partial A = X.
    $$ 
\end{dy}
...
\subsubsection{开集、闭集、邻域}

\begin{dy}[邻域]
    设 $(X, d)$ 是一个度量空间，$x \in X$，如果存在 $\varepsilon > 0$，使得 $B(x, \varepsilon)$，则称 $B(x, \varepsilon)$ 为 $x$ 的一个（开）邻域.
\end{dy}

\begin{dy}[开集]
    设 $(X, d)$ 是一个度量空间，$U \subset X$，如果对任意 $x \in U$，存在 $\varepsilon > 0$，使得 $B(x, \varepsilon) \subset U$，则称 $U$ 为 $X$ 中的一个开集. \\
    也即 $A = A^o$
\end{dy}

\begin{dy}[闭集]
    设 $(X, d)$ 是一个度量空间，$F \subset X$，如果 $X \setminus F$ 是一个开集，则称 $F$ 为 $X$ 中的一个闭集. \\
    也即 $A' \subseteq A$
\end{dy}

我们称 $\overline{A} = A \cup A'$ 为 $A$ 的闭包.

\begin{dl}
    设 $(X,d)$ 是一个度量空间. 以下命题等价：
    \begin{itemize}
        \item[(1)] $A \subset X$ 是一个闭集；
        \item[(2)] $A = \overline{A}$；
        \item[(3)] 若序列 $\{x_n\}_{n=1}^\infty \subset A$ 收敛，则其极限属于 $A$；
        \item[(4)] $A^c$ 是开集.
    \end{itemize}
\end{dl}

\begin{dl}
    任意个开集 (未必可数个) 的并是开集. 任意个闭集的交(未必可数个)是闭集. 有限个开集的交是开集. 有限个闭集的并是闭集.
\end{dl}

\begin{dl}
    设 $(X,d)$ 是一个度量空间，$A \subset X$。则：
    \begin{enumerate}
        \item[(1)] $\overline{A}$ 是包含 $A$ 的最小闭集。即：$\overline{A}$ 是闭集且对任意闭集 $K$，若 $A \subset K$，则必有 $\overline{A} \subset K$；
        \item[(2)] $A^\circ$ 是包含于 $A$ 的最大开集。即：$A^o$ 是开集且对任意开集 $U$，若 $U \subset A$，则必有 $U \subset A^o$。
    \end{enumerate}
\end{dl}
...
\begin{dy}[有限覆盖]
    设 $(X, d)$ 是一个度量空间，$A \subset X$，如果存在一族开集 $\{U_\alpha\}_{\alpha \in I}$，使得 $A \subset \bigcup_{\alpha \in I} U_\alpha$，且 $I$ 是有限集，则称 $\{U_\alpha\}_{\alpha \in I}$ 是 $A$ 的一个有限覆盖.
\end{dy}

\begin{dy}[紧集]
    设 $(X, d)$ 是一个度量空间，$A \subset X$，如果对任意 $A$ 的开覆盖 $\{U_\alpha\}_{\alpha \in I}$，都存在有限子覆盖 $\{U_{\alpha_i}\}_{i = 1}^n$，使得 $A \subset \bigcup_{i = 1}^n U_{\alpha_i}$，则称 $A$ 是一个紧集.
\end{dy}
...
\begin{zm}
    任取 $x \in A$, $\{{B}(x, n)\}_{n = 1}^\infty$ 构成 $A$ 的开覆盖. 有有限子覆盖 $\{{B}(x, n_i)\}_{i = 1}^k$. 则 $A \subset \bigcup_{i = 1}^k {B}(x, n_i) = {B}(x, n_k)$, 故 $A$ 有界.
\end{zm}

\subsubsection{列紧集}

\begin{dy}[列紧集]
    如果 $A$ 中的任意序列都有子列收敛到 $A$ 中元素，则称 $A$ 是列紧集.
\end{dy}

\begin{dl}\label{紧集是列紧集}
    紧集是列紧集.
\end{dl}

\begin{zm}
    反证法，设 $A$ 是紧集但非列紧集. 设 $\{x_n\}_{n=1}^\infty$ 是包含于 $A$ 的一个序列，没有任何子列收敛到 $A$ 中的元素. 先证明以下引理：
    \begin{yl}
        对任意 $x \in A$，存在 $\delta_x > 0$，使得 $B(x, \delta_x)$ 仅含 $\{x_n\}_{n=1}^\infty$ 中的有限项.
    \end{yl}
    \begin{ylzmhj}
        设引理为假，则存在 $x \in A$，使得对任意 $\delta > 0$，$B(x, \delta)$ 含有 $\{x_n\}_{n=1}^\infty$ 中的无限项. 于是，取 $\delta = 1, 1/2, 1/3, \ldots$，可以取 $x_{k_1} \in B(x, 1) \cap A$，$x_{k_2} \in B(x, 1/2) \cap A$ ($k_2 > k_1$)，$x_{k_3} \in B(x, 1/3) \cap A$ ($k_3 > k_2$)，$\ldots$ 由此得到子列 $\{x_{k_i}\}_{i=1}^\infty$ 收敛到 $x \in A$. 与 $\{x_n\}_{n=1}^\infty$ 没有任何子列收敛到 $A$ 中的条件矛盾. 引理得证.
    \end{ylzmhj}
    显然，$\{B(x, \delta_x)\}_{x \in A}$ 构成 $A$ 的开覆盖. 且由 $A$ 的紧致性，必有有限子覆盖 $\{B(x_{k_i}, \delta_{x_{k_i}})\}_{i=1}^N$. 由此，
    $$
        \{x_n\}_{n=1}^\infty \subset A \subset \bigcup_{i=1}^N B(x_{k_i}, \delta_{x_{k_i}})
    $$ 
    只有有限项，矛盾.
\end{zm}
...
\begin{dy}[完备集的定义]
    如果度量空间 $(X,d)$ 中的任意 Cauchy 序列收敛，则称该度量空间为完备空间.    
\end{dy}

\subsubsection{完备空间的性质}
...
\begin{dl}[闭集套定理]
    完备空间中的非空闭集列 $\{A_n\}_{n = 1}^\infty$ 构成套，且 $A_n$ 的直径 $\delta_n \to 0$. 其中 $\delta_n = \sup_{x, y \in A} d(x, y)$ \\
    则存在唯一 $\xi \in A_n, \forall n \in \mathbb{N}$.
\end{dl}

\begin{dy}[完全有界]
    $(X, d)$ 度量空间， $A \subseteq X$, 若对 $\forall \varepsilon > 0, \exists x_1, \cdots, x_n\ (\text{依赖于 } \varepsilon)$, 满足 \begin{align*}
        A \subsetneqq \bigcup_{i = 1}^{n}B(x_i, \varepsilon)
    \end{align*}
    则称 $A$ 完全有界，且称 $\{x_k\}_{k = 1}^n$ 为 $A$ 的一个 $\varepsilon$-网.
\end{dy}
...
\begin{dl}[紧性定理]
    $(X, d)$ 完备度量空间， $A \subseteq X$, 下列命题等价：
    \begin{itemize}
        \item [(1)] $A$ 是紧集.
        \item [(2)] $A$ 是列紧集.
        \item [(3)] $A$ 是完全有界闭集.
